...
class _SwipeAttendancePageState extends State<SwipeAttendancePage> {
  ...
  void _onSwipeEnd(
    ...
    setState(() {
      ...
      // Move overlay state to next top card and immediately reset swipe state
              ...
              child: GestureDetector(
                ...
                child: AppinioSwiper(
                  ...
                  onCardPositionChanged: (SwiperPosition position) {
                    ...
                    // Calculate target progress from the horizontal offset
                    // Scale it to match button press intensity (button uses 0.8)
                    final rawProgress = position.offset.dx / screenWidth;
                    final targetProgress =
                        rawProgress * 2.0; // Scale up to match button intensity

                    // Determine target swipe direction
                    String? targetDirection;
                    if (rawProgress.abs() > 0.0) {
                      targetDirection = rawProgress > 0 ? 'right' : 'left';
                    }

                    // Update immediately without animation to maintain color during finger movement
                    setState(() {
                      _swipeProgress = targetProgress.clamp(-2.0, 2.0);
                      _currentSwipeDirection = targetDirection;
                    });
                  },
                  cardBuilder: (BuildContext context, int index) {
                    ...
                    // Only calculate swipe progress for the current top card
                    double rawProgress = 0.0;
                    String? swipeDirection;
                    if (isTopCard) {
                      rawProgress = _swipeProgress.abs();
                      swipeDirection = _currentSwipeDirection;
                    }

                    // Smooth opacity curve: starts at 0, gradually increases
                    // Clamp values to prevent painting assertion errors
                    final swipeProgress =
                        rawProgress > 0.05 ? (rawProgress - 0.05) / 0.95 : 0.0;
                    final clampedSwipeProgress = swipeProgress.clamp(0.0, 1.0);
                    final isRightSwipe = isTopCard && swipeDirection == 'right';
                    final isLeftSwipe = isTopCard && swipeDirection == 'left';

                    Widget cardContent = Container(
                      decoration: BoxDecoration(
                        color: isRightSwipe
                            ? Colors.green.withOpacity(clampedSwipeProgress)
                            : isLeftSwipe
                                ? Colors.red.withOpacity(clampedSwipeProgress)
                                : Theme.of(context).scaffoldBackgroundColor,
                        borderRadius: BorderRadius.circular(20),
                      ),
                      child: Stack(
                        children: [
                          // Original student card content - keep it opaque to prevent bleed-through
                          Container(
                            decoration: BoxDecoration(
                          ...
                          if (isRightSwipe)
                            Positioned.fill(
                              child: Container(
                                decoration: BoxDecoration(
                                  color: Colors.green
                                      .withOpacity(clampedSwipeProgress),
                                  borderRadius: BorderRadius.circular(20),
                                ),
                                child: Center(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        'PRESENT',
                                        style: TextStyle(
                                          fontSize: (32 * clampedSwipeProgress)
                                              .clamp(0.0, 32.0),
                                          fontWeight: FontWeight.bold,
                                          color: Colors.white,
                                          letterSpacing: 2,
